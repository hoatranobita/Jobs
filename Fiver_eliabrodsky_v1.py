...
app.layout = html.Div([
    dcc.Tabs(id="tabs-example-graph", value='tab-1-example-graph', 
        children=[
        dcc.Tab(label='Dashboard', value='tab-1-example-graph'),
        dcc.Tab(label='Table', value='tab-2-example-graph'),
    ]),
    html.Div(id='tabs-content-example-graph')
])

# Create colorscales using to change heatmap colors
...
def update_line_graph(genes_type):
    if not genes_type or 'Select' in genes_type:
        ...
        #line_graph.write_html("line.html")
        return line_graph

app.run_server(port=8030,mode='inline') #You can delete mode='inline and dash will be open with link
