...
def update_data(n):
    ...
    jobs_2 = pd.read_sql(query, con = db)
    jobs_2.to_dict()
...
def update_graph(jobs_location,n):    
...
def update_graph_2(top_10,n):     
    jobs_title = pd.pivot_table(jobs_2,values=['Max Salary',],index=['Title'],aggfunc=np.mean)
...
def update_graph_3(jobs_rating,n):
    jobs_rating = pd.pivot_table(jobs_2,values=['Max Salary',],index=['Rating'],aggfunc=np.mean)
...
@app.callback(
    Output('location_cd_2', 'options'),
    [Input('store-data','data'),Input('update', 'n_intervals')])

def options(search_value,n):
        return [{'label':i,'value':i} for i in jobs_2.sort_values('Location')['Location'].unique()]
...
def build_graph(location_code,dff_2):
    if not location_code or 'Select' in location_code:
        dff_2 = pd.pivot_table(jobs_2,values=['Max Salary'],index=['Location','Title'],aggfunc=np.mean).reset_index()
...
@app.callback(dd.Output('image_wc', 'src'), [dd.Input('image_wc', 'id'),dd.Input('update', 'n_intervals')])
def make_image(b,n):
    img = BytesIO()
    job_cloud = jobs_2.groupby(["Title"])["Title"].count().reset_index(name="count")    
